...
flowchart TB
    ...
    C{ノートオンが来たら空きチャンネルを探して割当て（チャンネル数は４つ）}--> D[パルス発生] --> H[モータードライバ] --> L[モーター１]
    C{ノートオンが来たら空きチャンネルを探して割当て（チャンネル数は４つ）}--> E[パルス発生] --> I[モータードライバ] --> M[モーター２]
    C{ノートオンが来たら空きチャンネルを探して割当て（チャンネル数は４つ）}--> F[パルス発生] --> J[モータードライバ] --> N[モーター３]
    C{ノートオンが来たら空きチャンネルを探して割当て（チャンネル数は４つ）}--> G[パルス発生] --> K[モータードライバ] --> O[モーター４]
```
...
## 音階
１オクターブは、振動数で２倍の関係にあります。例えば高い「ド」の振動数は、低い「ド」の振動数の２倍になります。この関係を１２に同じ比率に分割した音階が普通に使われています（平均律）。
おもしろいことに、「ド」と「ソ」の関係は、きれいな振動数の比（２対３）になるので、同時に鳴らすとハモリます。
